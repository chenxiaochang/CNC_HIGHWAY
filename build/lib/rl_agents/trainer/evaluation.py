...
class Evaluation(object):
    ...
    def train(self):
        self.training = True
        if getattr(self.agent, "batched", False):  # 返回一个对象属性值
    ...
    def after_all_episodes(self, episode, rewards):
        rewards = np.array(rewards)   # 奖励数组化
        gamma = self.agent.config.get("gamma", 1)  # 外部获取gamma
        # velocity = self.agent.
        self.writer.add_scalar('episode/length', len(rewards), episode)  # 将所需数据保存在文件里供可视化
        self.writer.add_scalar('episode/total_reward', sum(rewards), episode)
        self.writer.add_scalar('episode/return', sum(r*gamma**t for t, r in enumerate(rewards)), episode)  # 累积奖励
        self.writer.add_histogram('episode/rewards', rewards, episode)
        # self.writer.add_scalar('episode/velocity',len(),episode)  可视化车辆的实时速度
