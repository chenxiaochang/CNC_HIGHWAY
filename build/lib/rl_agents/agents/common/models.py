...
class BaseModule(torch.nn.Module):  # definition one baseModule class which inherit the nn.module
    """
    ...
    def __init__(self, activation_type="RELU", reset_type="XAVIER", normalize=None):  # 激活函数RELU，采用xavier初始化
        super().__init__()
        self.activation = activation_factory(activation_type)
        self.reset_type = reset_type  # 初始类型 参数输入维度n，输出维度m，参数以均匀分布形式存在[-sqrt(6/(n+m),+sqrt(6/(n+m))]
        self.normalize = normalize
        self.mean = None
        self.std = None

    def _init_weights(self, m):   # 初始化权重矩阵
        if hasattr(m, 'weight'):  # hasattr(object,name):判断对象是否包含对应的属性
    ...
    def set_normalization_params(self, mean, std):  # definition weather the parameter should be normalization
        if self.normalize:
            std[std == 0.] = 1.
        self.std = std
        self.mean = mean
...
class MultiLayerPerceptron(BaseModule, Configurable):  # 多层感知机---全连接网络   需要继承父类nn.module
    def __init__(self, config):
        super().__init__()
        Configurable.__init__(self, config)
        sizes = [self.config["in"]] + self.config["layers"]  # sizes=[None,64,64] None equal the size of batch
        self.activation = activation_factory(self.config["activation"])  # RELU
        layers_list = [nn.Linear(sizes[i], sizes[i + 1]) for i in range(len(sizes) - 1)]  # 设置全连接层，输入输出均为二维张量
        """len(sizes)=3, layers_list=[nn.linear(none,64),nn.linear(64,64)]"""
        self.layers = nn.ModuleList(layers_list)  # 利用此list会将module注册到整个网络上，其参数也会自动添加到整个网络上
        """nn.modulelist 用来存储任意数量的nn.moudule,此处用来存储两层全连接结构层，大小为[in,64]|[64,64]"""
        if self.config.get("out", None):
            self.predict = nn.Linear(sizes[-1], self.config["out"])  # nn.linear([64,None])
    ...
    """前向传播将输入信号  -> 输出产生输出误差，通过反向传播误差信息更新系统权重矩阵，权重在双向的信息流中得到优化"""

    def forward(self, x):  # nn.sequential内部实现了forward函数，nn.ModuleList没有实现内部forward函数，需要自己写
        if self.config["reshape"]:  # True
            x = x.reshape(x.shape[0], -1)
        """x.shape[0],read row |x.shape[1],read column    reshape(-1,1)->reshape(任意行,1列)"""
        for layer in self.layers:
            x = self.activation(layer(x))  # 两层FC均使用RELU
        if self.config.get("out", None):
            x = self.predict(x)
        return x


class DuelingNetwork(BaseModule, Configurable):  # with the advantage function A=Q-V;
    def __init__(self, config):
        super().__init__()
        Configurable.__init__(self, config)
        self.config["base_module"]["in"] = self.config["in"]  # 输入多层感知机，层数256,128
    ...
    def forward(self, x):
        ...
        return value + advantage - advantage.mean(1).unsqueeze(1).expand(-1, self.config["out"])  # unsqueeze在第二维增加一个维度


class ConvolutionalNetwork(nn.Module, Configurable):  # CNN
    def __init__(self, config):
        super().__init__()
        Configurable.__init__(self, config)
...
class EgoAttention(BaseModule, Configurable):  # 自注意力机制
    def __init__(self, config):
        super().__init__()
        Configurable.__init__(self, config)
        self.features_per_head = int(self.config["feature_size"] / self.config["heads"])  # 64/2=32
        # class torch.nn.linear(in_feature,out_feature,bias=Ture)
        self.value_all = nn.Linear(self.config["feature_size"], self.config["feature_size"],
                                   bias=False)  # FCN[64,64] Input and output should be two-dimension tensor
        self.key_all = nn.Linear(self.config["feature_size"], self.config["feature_size"], bias=False)  # 64*64
        self.query_ego = nn.Linear(self.config["feature_size"], self.config["feature_size"], bias=False)  # y=k*x
        self.attention_combine = nn.Linear(self.config["feature_size"], self.config["feature_size"],
                                           bias=False)  # the output of  head value be combined by linear layer 64*64
    ...
    def default_config(cls):
        return {
            "feature_size": 64,
            "heads": 4,
            "dropout_factor": 0,  # 丢弃法dropout常用来应对过拟合问题，丢弃概率是一超参数，加在FC后面
        }

    def forward(self, ego, others, mask=None):  # 对一层节点和对应连接进行加权和运算
        batch_size = others.shape[0]  # 读取第一维度的张量大小    行 数  1
        n_entities = others.shape[1] + 1  # 第二维长度     列 数 + 1= 14+1
        input_all = torch.cat((ego.view(batch_size, 1, self.config["feature_size"]), others),
                              dim=1)  # view改变tensor的形状，cat按维数1拼接，横着拼接，行数必须一致 ——>一行张量
        # Dimensions: Batch, entity, head, feature_per_head       key_all =[1,15,2,32]
        ...
        # Dimensions: Batch, head, entity, feature_per_head  1 2 15 32
        key_all = key_all.permute(0, 2, 1, 3)  # 用于维度换位  1 2 15 32
        value_all = value_all.permute(0, 2, 1, 3)  # 1 2 15 32
        query_ego = query_ego.permute(0, 2, 1, 3)  # 1 2 1 32
        if mask is not None:  # mask=[1,1,1,15]
        ...
        value, attention_matrix = attention(query_ego, key_all, value_all, mask,
                                            nn.Dropout(self.config["dropout_factor"]))  # output,p_attn
        # result=输出的Value*线性变换 [1,64] + 残差网络[1,1,64]去除第二维->[1,64]的线性组合
        # 对神经网络来说，恒等映射并不容易拟合，很好地解决退化问题
        result = (self.attention_combine(value.reshape((batch_size, self.config["feature_size"]))) + ego.squeeze(1)) / 2
        return result, attention_matrix  # result如上，attention_matrix=p_attn->QKt
...
class EgoAttentionNetwork(BaseModule, Configurable):
    def __init__(self, config):
        ...
        Configurable.__init__(self, config)
        self.config = config
        if not self.config["embedding_layer"]["in"]:  # ["in"]=7;
    ...
    def split_input(self, x, mask=None):  # 切分输入
        # Dims: batch, entities, features  1  15 64
        ego = x[:, 0:1, :]  # 0:1 含左不含右，取出第二个维度的第一行
        others = x[:, 1:, :]  # 含左不含右，取出第二个维度的第二行到后面所有行
        if mask is None:
    ...
    def forward_attention(self, x):
        ...
        return self.attention_layer(ego, others, mask)  # [1,64]

    def get_attention_matrix(self, x):
        _, attention_matrix = self.forward_attention(x)  # _ 取出第一维的信息,attention_matrix 取出原来的输入矩阵
        return attention_matrix  # [1,64]
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    """
    d_k = query.size(-1)  # the number of features in the query
    # print(d_k)   scores=[1,2,1,15]
    scores = torch.matmul(query, key.transpose(-2, -1)) / np.sqrt(d_k)  # tensor multiple  transpose 转置
    if mask is not None:
        scores = scores.masked_fill(mask, -1e9)
    p_attn = F.softmax(scores, dim=-1)  # p_attn=[1,2,1,15]
    if dropout is not None:
        p_attn = dropout(p_attn)
    output = torch.matmul(p_attn, value)  # matrix multiple
    return output, p_attn  # output—>QKT *V p_attn->QKT
...
def activation_factory(activation_type):
    if activation_type == "RELU":
        ...
        return torch.tanh
    elif activation_type == "Sigmoid":  # 自己加的
        return torch.sigmoid
    else:
        raise ValueError("Unknown activation_type: {}".format(activation_type))


def trainable_parameters(model):  # 返回net中的总参数 [model.parameters()  iteration back all parameters in models]
    return sum(p.numel() for p in model.parameters() if p.requires_grad)  # numel():返回数组中元素个数
...
def size_model_config(env, model_config):
    ...
    """
    if model_config["type"] == "ConvolutionalNetwork":  # Assume C*H*W observation space
        model_config["in_channels"] = int(env.observation_space.shape[0])  # 读取第一维 channels
        model_config["in_height"] = int(env.observation_space.shape[1])    # 读取第二维 height
        model_config["in_width"] = int(env.observation_space.shape[2])     # 读取第三维 width
    else:
        model_config["in"] = int(np.prod(env.observation_space.shape))  # np.prod 计算所有元素的乘积
    model_config["out"] = env.action_space.n  # [1,3]
