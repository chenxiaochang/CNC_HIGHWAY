...
def agent_factory(environment, config):
    ...
    if "__class__" in config:
        ...
        ''' "__class__": "<class 'rl_agents.agents.deep_q_network.pytorch.DQNAgent'>"-> rl_agents.agents.deep_q_network.pytorch.DQNAgent '''
        module_name, class_name = path.rsplit(".", 1)  # 1 rl_agents.agents.deep_q_network.pytorch    2 DQNAgent
        agent_class = getattr(importlib.import_module(module_name), class_name)  # import_module动态导入模块， getattr返回一个对象的属性
