...
class RunAnalyzer(object):
    ...
    def get_run_dataframe(self, directory, agent_name='', gamma=0.95, subsample=10):
        ...
        data = {
            ...
            "discounted rewards": [np.sum([episode[t] * gamma ** t for t in range(len(episode))])
                                   for episode in run_data["episode_rewards_"]],  # episode from the number 1
        ...
        try:
            dt = 1.0
            data.update({  # update:键值对的添加-> data
                "crashed": [np.any(episode) for episode in run_data["episode_crashed"]],
                "speed": [np.mean(episode) for episode in run_data["episode_speed"]],  # from OpenAiGym episodes_batch
                '''将speed 处理成[1,2,3,4,5]的形式,horizon = 5'''
    ...
    def analyze(self):
        self.find_best_run()
        for field in ["total reward", "length", "speed", "distance", "crashed"]:
            logging.info("Analyzing {}".format(field))
            fig, ax = plt.subplots()
            sns.lineplot(x="episode", y=field, hue="agent", data=self.data, ax=ax, ci=95)  # hue:data数据名|ci=置信区间大小
            field_path = self.out / "{}.pdf".format(field)
            fig.savefig(field_path, bbox_inches='tight')  # 去除空白
            plt.show()
            plt.close()
